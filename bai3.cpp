...
{ // return 2
    ...
    if (x == 13)
    {
        return Xorbin(Xorbin(Get2From16(xtime(xtime(temps))), Get2From16(temps)),
...
void Nhap()
{
    ...
    for (int i = 1; i <= 4; i++)
    {
        ...
        {
            char temp = sk[ii];
            // cipherkey ở đây lấy 1 từ theo hàng à? Ở link nguyenquan lấy theo cột
...
void init()
{
    ...
    // fin.close();
    // cipher
    // fstream fin1("cipherkey.txt",ios::in);
    // for(int i=1;i<=4;i++){
    // for(int j=1;j<=4;j++){
    // fin1>>cipherkey[i][j];
    // }
...
void addRoundKey()
{
    ...
    { // dong thu i
        ...
        // thay thế bởi giá trị trong sbox
        s = sbox[x][y];

        // thực hiện AddRcon, cũng là tính w[4/8/12...] (cột 1) luôn
        string nptemp = Get2From16(cipherkey[i][1]);
        string nps = Get2From16(s);
        string nprconi = Get2From16(rconi[i]);
        string tempda = nps;
        // cout<<i<<endl<<temp[i]<<endl<<nptemp<<endl<<nps<<endl<<nprconi<<endl;
    ...
    //===========================================
    // cột 1 tính ở trên rồi, nên giờ tính cột 2-4
    for (int i = 2; i <= 4; i++)
    {
        for (int j = 1; j <= 4; j++)
        {
